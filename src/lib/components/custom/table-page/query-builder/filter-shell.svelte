<script lang="ts">
	...
	const fieldLabel = $derived(getField(fields, filter.field)?.label ?? 'Field');
	const ops = $derived(operators[filter.type]);
	const operatorLabel = $derived(ops.find((o) => o.value === filter.operator)?.label ?? 'Operator');
</script>
...
<div class="flex items-center gap-2">
	...
	<Select.Root type="single" value={filter.operator} onValueChange={changeOperator}>
		...
		<Select.Content>
			<Select.Group>
				<Select.Label>Operators</Select.Label>
	...
	<Button variant="outline" size="icon-sm" onclick={onDelete} aria-label="Remove filter">
		<XIcon />
	</Button>
</div>
