...
<Sheet.Root bind:open>
	<Sheet.Content class="gap-0">
		<Sheet.Header>
			<Sheet.Title>{m.event_dialog_delete_title()}</Sheet.Title>
			<Sheet.Description>
				{m.event_dialog_delete_description({ event: event?.event ?? '' })}
			</Sheet.Description>
		</Sheet.Header>
		...
		<Sheet.Footer class="flex-row justify-end gap-2">
			...
			<Button variant="destructive" onclick={handleConfirm} disabled={!event}>
