<script lang="ts">
	...
	import { type FanAction, tags } from '../columns';
	import { ChevronsUpDownIcon, X, Check } from '@lucide/svelte';

	import * as m from '$lib/paraglide/messages.js';

	let { eventToEdit = $bindable() }: { eventToEdit: FanAction | null } = $props();
	...
	$effect(() => {
		if (eventToEdit) {
			...
			selectedTags = [...(eventToEdit.tags ?? [])];
			isActive = eventToEdit.active ?? false;
		}
	});
